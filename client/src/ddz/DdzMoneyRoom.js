 ...
 * @extend {Room}
 */
var DdzMoneyRoom = DdzRoom.extend({
	...
	ctor:function(){
		...
		this._dt = 0;
		this._awardDt = 0;
		this._countDown = DdzRoomModel.getTuoguanTime();//this.COUNT_DOWN;
		this._loacationDt = 0;
		this.countDownLabel = null;
		this._timedt = 0;
		this._super(LayerFactory.DDZ_MONEY_ROOM);
	},
	...
	initData:function(){
		...
		//this.updateCountDown(DdzRoomModel.getTuoguanTime());//this.COUNT_DOWN);
		...
		for(var i=0;i<players.length;i++){
			...
			if(p.userId ==PlayerModel.userId){//自己的状态处理
				...
				if(this.bg_CancelTuoguan){
					var isMeTuoguan = DdzRoomModel.getPlayerIsTuoguan(p);
	...
	dealMoneyView:function(){
		var isMoneyRoom = DdzRoomModel.isMoneyRoom();
		if(isMoneyRoom){
			cc.log("当前是金币场房间！！！！");
			this.btnStopMoneyRoom = this.getWidget("btnStopMoneyRoom");
			this.tempLable3 = this.getWidget("lbCoin_0");
			this.lableRoomId = this.getWidget("lbCoinBeilv");
			this.tempLable3.setString("娱乐场");
			this.lableRoomId.setString(DdzRoomModel.getMoneyRoomBeilv() +"倍");
	...
	playOrRemoveWaitingAnm:function(){
		...
		showOrHideNode.push(this.getWidget("Button_55"));//设置按钮
		showOrHideNode.push(this.getWidget("Image_7"));
		...
		showOrHideNode.push(this.getWidget("Image_54"));
		//showOrHideNode.push(this.getWidget("Button_30"));
		showOrHideNode.push(this.getWidget("Label_version"));//版本号
		showOrHideNode.push(this.tempLable3);
		showOrHideNode.push(this.lableRoomId);
		//showOrHideNode.push(this.fapai);
		//showOrHideNode.push(this.Button_21);
		...
		var alwaysHideNode = [];
		alwaysHideNode.push(this.Button_17);
		alwaysHideNode.push(this.getWidget("Label_jushu"));
		alwaysHideNode.push(this.getWidget("Button_30"));
		alwaysHideNode.push(this.getWidget("Label_38"));
		alwaysHideNode.push(this.Button_21);
		...
		//人数不够 未开始的情况下
		if(DdzRoomModel.players.length < DdzRoomModel.renshu){
	...
	updatePlayTuoguan:function(event){
		...
		if(data.length >= 2){
			...
			if(seat == DdzRoomModel.mySeat && this.bg_CancelTuoguan){
	...
	{
		if (this.dzcard){
			this.dzcard.removeFromParent(true);
			this.dzcard = null;
		}
	...
	onOver:function(event){
		var data = event.getUserData();
		if(PlayDDZMessageSeq.sequenceArray.length>0){
			PlayDDZMessageSeq.cacheClosingMsg(data);
			return;
		}
		...
		setTimeout(function(){//延迟弹出结算框
			...
			var mc = new DdzMoneyResultPop(data);
	...
	onLetOutCard:function(event){
		this._countDown = DdzRoomModel.getTuoguanTime();
	...
	onJoin:function(event){
		...
		if(DdzRoomModel.isMoneyRoom()){
			if(p.userId == PlayerModel.userId){
				DdzRoomModel.mySeat = p.seat;//修正错误的mySeat值
			}
		}
		var seq = this.getPlayerSeq(p.userId,DdzRoomModel.mySeat, p.seat);
		...
		if(DdzRoomModel.isMoneyRoom() && ObjectUtil.size(this._players) >= DdzRoomModel.renshu){
			var beginMsg = PlayDDZMessageSeq.getBeginMsg();
			if(beginMsg){
	...
	startGame:function(event){
		...
		if(isMoneyRoom){//金币场 在人数没有显示完全的时候 要把开始游戏消息缓存下来
			if(ObjectUtil.size(this._players) < DdzRoomModel.renshu){
				PlayDDZMessageSeq.cacheBeginMsg(message);
		...
		for(var i=1;i<=3;i++){
			if(i>1)
				this.getWidget("bt"+i).visible = false;
			this.getWidget("ybq"+i).visible = false;
			this.getWidget("xz"+i).visible = false;
