 ...
 */
//半边天炸详情的弹框
var ClubBbtzRecallDetailPop = BasePopup.extend({
    ctor: function (data) {
        this.data = data;
        this._super("res/clubBbtzRecallDetailPop.json");
    },
    ...
    selfRender: function () {
        ...
        for(var i=0;i<playLog.length;i++){
            var item = new ClubBbtzRecallDetaiItem(playLog[i].playerMsg.length, this.checkUserId);
...
//半边天炸详情的弹框的Item
var ClubBbtzRecallDetaiItem = ccui.Widget.extend({
    data:null,
    ...
    onHuiFang:function(){
        ...
        for (var i = 0; i < PopupManager.popupList.length; i++) {
            ...
            } else if (PopupManager.popupList[i]["constructor"] == ClubBbtzRecallDetailPop) {
                ClubRecallDetailModel.isShowRecord = true;
            }
        }
        //PopupManager.removeAll();
        PopupManager.hidePopup(ClubRecallPop);
        PopupManager.hidePopup(ClubBbtzRecallDetailPop);
        PopupManager.hidePopup(ClubHomePop);
        if(this.data.playType==131){
