 ...
 */
//红中详情的弹框
var ClubSymjRecallDetailPop = BasePopup.extend({
    ctor: function (data) {
        this.data = data;
    ...
    selfRender: function () {
        ...
        for(var i=0;i<playLog.length;i++){
            var item = new ClubSymjRecallDetailItem(playLog[i].playerMsg.length, this.checkUserId);
...
//跑胡子详情的弹框的Item
var ClubSymjRecallDetailItem = ccui.Widget.extend({
    data:null,
    ...
    onHuiFang:function(){
        ...
        for (var i = 0; i < PopupManager.popupList.length; i++) {
            ...
            } else if (PopupManager.popupList[i]["constructor"] == ClubSymjRecallDetailPop) {
        ...
        PopupManager.hidePopup(ClubSymjRecallDetailPop);
        //cc.log("this.renLength.::::::::="+this.renLength)
       if(this.data.playType==MJWanfaType.SYMJ) {
