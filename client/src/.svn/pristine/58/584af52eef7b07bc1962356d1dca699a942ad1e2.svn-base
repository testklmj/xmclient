...
var PDKRoom = BaseRoom.extend({//Room
	...
	ctor:function(){
		this._statusMap = {};
		this._dt = 0;
		this._countDown = PDKRoomModel.getTuoguanTime() || this.COUNT_DOWN;
		this._timedt = 0;
		this._loacationDt = 0;

		this.showTuoGuanTimeOutHandle = 0;
	...
	selfRender:function(){
		...
		this._cardPanelAni.visible = true;
		...
		if (PDKRoomModel.roomName){
			...
			this.roomName_label.y = cc.winSize.height/2 + 290;
		}
	...
	onShowVoiceAndProps:function(){
		if (PDKRoomModel.isBanVoiceAndProps()){
	...
	updatePlayTuoguan:function(event){
		...
		if(data.length >= 2){
			//var userId = data[0];
				...
				var player = self._players[seat];
				if(player){
					player.updateTuoguan(isTuoguan);
				}else{
					cc.log("!!!!!!!未获取到player");
				}
				if(seat == PDKRoomModel.mySeat && self.bg_CancelTuoguan){
					if (isTuoguan){
	...
	startGame:function(event){
		...
		var p = event.getUserData();
		this._players[PDKRoomModel.mySeat].deal(p.handCardIds);
		this._countDown = PDKRoomModel.countDown;
		this.showJianTou();
		this._lastCardPattern = null;
